...
# -*- coding: utf-8 -*-
"""
Page 2 — Performances de l'IA (BERT)
...
for col, color, label, value, delta in [
    (k1, "blue",   "PRÉCISION MOY.",   f"{avg_precision:.1%}", f"dernière: {latest['précision']:.1%}"),
...
for col_name, color, name, dash in [
    ('précision', COLORS['blue'], 'Précision', 'solid'),
...
# Zone cible (seuil acceptable)
fig.add_hrect(y0=0.85, y1=1.0, fillcolor='rgba(5,122,85,0.04)',
              line_width=0, annotation_text="Zone cible (>85%)",
              annotation_position="bottom right",
              annotation_font=dict(size=10, color='#057a55'))

layout = PLOTLY_LAYOUT.copy()
layout.update(
    height=300,
    yaxis=dict(**PLOTLY_LAYOUT['yaxis'],
               tickformat='.0%', range=[0.75, 1.0]),
...
with c2:
    ...
    layout3.update(
        ...
        yaxis=dict(**PLOTLY_LAYOUT['yaxis'], showgrid=False),
        margin=dict(l=0, r=60, t=30, b=0)
    )
    fig3.update_layout(**layout3)
    st.plotly_chart(fig3, use_container_width=True, config={'displayModeBar': False})

# ─── VOLUME DE CLASSIFICATION ───────────────────────────────────────────────────
